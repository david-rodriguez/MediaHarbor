...
1. Do steps 1 to 3 of the [README setup](../README.md#set-it-up). Keep the clone at `/opt/mediaharbor` on the host.
2. Run `./harbor check`, then `sudo ./harbor prepare`. Do not run `./harbor compose up`.
...
6. Under **Environment variables**, select **Load variables from .env file** and pick your `config/host.env`. If the file does not set `HARBOR_ROOT`, add it with the value `/opt/mediaharbor`.
...
8. Do steps 5 to 9 of the README setup. If the Plex token expired, put a new token in `secrets/plex_claim` and restart the `plex` container in Portainer.
